...
created: 2026-08-28
updated: 2026-09-15
revision_note: "Final PRD reconciliation and cleanup complete (2026-09-15). All locked Brief requirements resolved. Identifiers preserved (FR-1..FR-50, UJ-1..UJ-5, D1..D8, A1..A6, OQ-1..OQ-5). Found/Lost audit separation, 8-status Recovery Response model, owner-determines / Administrator-records verification, per-Claim and per-Recovery-Response threads, four administrator queues, eight MVP email events, and FR-37/FR-39 visibility model all final."
---
...
**Revision 2 (2026-09-15):** The Product Brief was finalized after the PRD's first pass, adding the Lost-report **Recovery Response** capability and the **Browse / My Reports** listing partition for authenticated members. Revision 2 integrates those additions while preserving the existing structure: FR-1 through FR-39, UJ-1 through UJ-4, decisions D1 through D4, assumptions A1 through A6, and open questions OQ-1 through OQ-5 are preserved unchanged where unaffected by the new Brief. New requirements (FR-40 through FR-49), a new journey (UJ-5), new glossary terms, new audit events, new decisions (D5 through D7), and new success metrics are added in dedicated subsections. Claim behavior is intentionally not merged with Recovery Response behavior — they are separate domain concepts and remain so throughout.

**Revision 3 (2026-09-15, in-place correction pass):** A reconciliation pass resolves internal inconsistencies discovered during review. The overall Recovery Response direction introduced in Revision 2 is correct and is preserved. Identifiers (FR-1..FR-50, UJ-1..UJ-5, D1-D8, A1-A6, OQ-1..OQ-5) are preserved; no renumbering. Substantive corrections:

- **Canonical Recovery Response statuses (final, 8 statuses):** Submitted, Selected for Verification, **Match Confirmed**, Not a Match, **Completed — Report Returned**, Resolved — Report Returned, Resolved — Report Closed, Withdrawn. **Standby is a derived/display condition** for a Submitted response when another response is currently selected — never a persisted status. **Match Confirmed is NOT terminal** — it means the Lost-report owner has determined the candidate is theirs while physical return is still pending. **Completed — Report Returned** is the new successful terminal status reached when Match Confirmed + physical handoff + administrator Confirm Returned all occur.
- **Owner determines / Administrator records:** The Lost-report owner physically determines Match Confirmed vs Not a Match; the acting administrator records the determination in the system. The owner does not need to click in the application. The administrator does not independently decide whether the item belongs to the owner.
- **Lost-report lifecycle:** Open → Verification Pending → {Open (Not a Match) | Returned (Match Confirmed + Confirm Returned) | Closed (reporter withdrawal pre-Match-Confirmed OR documented exceptional administrator cancellation after Match Confirmed)}. Verification Pending is the only Lost-only intermediate status. Claim Requested and Claim Approved are Found-only intermediate statuses. Open, Returned, and Closed are shared.
- **Listing partition + participation views (D5 + D8):** My Reports (reports I authored), Browse (other members' reports), My Claims (Claims I submitted), My Recovery Responses (Recovery Responses I submitted). D8 is exclusively the participation-views decision; the Standby rule lives in D7, FR-41, and the glossary.
- **Thread privacy:** **One message thread per Recovery Response** (not per Lost report). Each thread is scoped to that single Recovery Response; participants are the Lost-report owner, that responder, and all administrators. Competing responders cannot see each other's threads. The Lost-report owner can see every per-Recovery-Response thread on their own Lost report.
- **Visibility model:** A single authoritative visibility matrix by actor (FR-39) replaces the FR-13/FR-37/FR-38/FR-39 contradictions. FR-38 is the anonymous-only summary set; FR-39 is the authenticated detail visibility matrix.
- **Edit blocklist (FR-9):** Reporter edit blocked once report reaches Returned or Closed (both terminal).
- **Deletion/withdrawal symmetry (FR-10/FR-11):** Hard delete only for interaction-free Open reports (Found requires no Claim; Lost requires no Recovery Response); otherwise Withdraw → Closed. Withdrawal rules differ by report type (correction #5): Found allowed on Open / Claim Requested only; Lost allowed on Open / Verification Pending pre-Match-Confirmed only. Withdraw is blocked once a Found report reaches Claim Approved and once a Lost report reaches Match Confirmed, Returned, or Closed.
- **Terminal-state responsibility (FR-22):** Returned is reachable only via administrator confirmation of physical return. Closed is reachable via permitted reporter withdrawal, administrator moderation, exceptional post-Match-Confirmed administrator cancellation, or organization-removal.
- **Notification model:** Eight MVP email events (FR-27, FR-28, FR-29, FR-46 events 4-8). FR-30 reframed as "Unsupported Notification Channels and Events". FR-46 event 8 (Successful match) fires only after Completed — Report Returned + Lost report Returned, not at Match Confirmed.
- **Registration notifications (FR-1, FR-3, UJ-3):** Account-approved email on approval; no registration-rejection email; no separate in-app registration event.
- **Claim privacy (UJ-2, FR-37, FR-39):** Claim reason and identifying details are visible only to the claimant and administrators. The Found-report owner/finder sees the Found-report status, the per-Claim thread, and the New-Claim email, but does NOT see claimant identifying details or claimant reason. The finder does not evaluate the claimant's private ownership evidence.
- **Claim notifications (FR-29):** Claim-decision email is sent to the claimant only.
- **Match Confirmed → handoff edge case:** Once administrator records Match Confirmed, reporter withdrawal is no longer permitted; the workflow must proceed to administrator Confirm Returned. Administrator cancellation after Match Confirmed is permitted only with a documented exceptional reason.
- **Administrator queues (FR-33):** Four queues: Pending Registrations, Pending Claims, Items Awaiting Return (Found-side), Items in Verification (Lost-side, FR-50). Audit is not one of the four operational queues.
- **New FR-50:** Administrator Items-in-Verification queue (Lost reports in Verification Pending).
- **Audit trail (FR-48):** Events 18 (Match Confirmed) and 19 (Not a Match) record both `determined_by` (Lost-report owner) and `recorded_by` (acting administrator); events 15-30 cover the full Lost-side trail. Event 26 is the parent Lost-report-level batch Returned transaction; Event 30 is the individual matched selected Recovery Response terminal transition to `Completed — Report Returned` (fired in the same transaction).
- **Responder withdrawal:** Withdrawer-only while Submitted and not Selected for Verification.
- **Recovery Response resubmission:** New Recovery Response record after Not a Match; original preserved.
- **OQ-4 retired** into a "Resolved / Retired Questions" subsection.
- **Found-side administrative closure after Claim Approved (FR-21, correction #21):** Exceptional admin Close allowed with documented reason; Approved Claim remains Approved; audit records incomplete return.
- **Found-side administrator cancellation before Claim Approved:** FR-21 still permits administrator Close on Open or Claim Requested Found reports.
- **Lost-report owner deactivation (correction #14):** Open → admin Close; VP pre-Match → may continue with admin recording owner determination; post-Match → admin handoff or exceptional cancellation.
- **Sensitive Recovery Response fields** are protected at the API layer.
- **Edit traceability (correction #23):** Once any Claim or Recovery Response exists, every edit is audit logged with snapshot of changed fields, actor, timestamp.
...
A Claim and a Recovery Response are not the same thing. Each has its own form fields, lifecycle states, selection rules, and audit trail. The MVP keeps them separate throughout.
...
- *Path:*
  ...
  7. An administrator reviews the claim and approves it. The Claim status transitions to `Approved`; the Found report status transitions to `Claim Approved`. **Maya receives the "claim approved" email (FR-29).** **No Claim-decision email is sent to the finder** (correction #9). Other Pending Claims on the same Found report are auto-rejected with reason "another claim was approved for this item" (FR-17).
...
- *Path:*
  ...
  3. Three days later, Sam receives an email: a new claim has been submitted on his found item (FR-28). He opens the item and sees that a Claim exists (correction #9). **He does NOT see the claimant's reason or identifying details** (correction #9; FR-37, FR-39); those are visible only to the claimant and administrators. Sam can see the Found-report status and post in the per-Claim thread, but he does not evaluate the claimant's private ownership evidence — the administrator evaluates it.
  4. Sam does not respond directly. He trusts the administrator's review.
  5. The administrator approves Maya's claim. **Sam does NOT receive a Claim-decision email** (correction #9; only the claimant receives FR-29). He sees the item status change to `Claim Approved` the next time he visits his My Reports.
...
- *Edge case:* if Sam edits his Found report before any Claim exists, the edit is recorded with a timestamp in the audit trail (FR-19). If Sam tries to edit it after it reaches `Returned` or `Closed`, the edit is blocked with a clear message (FR-9, correction #3).
...
- *Path:*
  ...
  2. The account is created in `Pending` status. Alex sees a confirmation screen that says an administrator will review the request. **No email is sent** (FR-1, correction #8).
  3. The registration appears in the **Pending Registrations** queue (FR-3). **No separate in-app notification event fires for administrators** (correction #8; administrators learn of new registrations by visiting the queue, not via an in-app notification banner).
...
- *Edge case — Rejection:* If Alex's account is rejected, no rejection email is sent (correction #8). The account is set to `Rejected`. Alex's record is preserved for audit but he cannot log in. The rejection reason is visible only to administrators in the audit trail. Alex is informed of his rejection status only if he attempts to log in (FR-4 generic "your account is not active" message).
...
- *Path:*
  ...
  7. Riley reviews the two pending registrations, approves one (sends the approval email FR-27 and activates the account), and rejects the other with a recorded reason (no rejection email — correction #8).
  8. Riley switches to the **Items in Verification** queue (FR-50), opens the UJ-5 Lost report in `Verification Pending`, sees every Recovery Response (selected and the standby pool), opens the verification screen (FR-45), and arranges the physical verification at the desk. The acting administrator on each verification action is Riley (correction #36). **At the desk, the Lost-report owner (Maya) physically inspects the candidate item and determines whether it is hers; Riley records Maya's determination in FindBack on her behalf** (correction #3; FR-43 owner-determines / FR-45 admin-records). The audit entry records `determined_by = Maya` and `recorded_by = Riley`, with the determination itself and the verification timestamp.
...
- *Edge case:* if Riley tries to edit a report that is already `Returned` or `Closed`, the edit is blocked with a clear message (FR-9, correction #3).

**UJ-5. Maya recovers her lost wallet after Sam submits a Recovery Response (Lost-side flow, revision 3 corrected).**
...
- *Path:*
  ...
  3. Sam opens the Lost report detail page (the report is not his own), reads the **authenticated-visible** description (FR-39; Sam does **not** see Maya's private identifying-details text — correction #35), and clicks **Submit Recovery Response**.
  ...
  7. Maya selects Sam's response by clicking **Select for Verification**. The system records the selection event (FR-48 event 17); Sam's response moves to `Selected for Verification`. The Lost report transitions from `Open` to `Verification Pending` (FR-42, FR-43). **Pat's response stays in `Submitted`** but the UI renders it as `Submitted — Standby` while Sam's response is selected — `Standby` is a **display-only condition**; Pat's canonical persisted status remains `Submitted` (correction #2). Pat's per-Recovery-Response thread remains writable because Pat's response is still `Submitted`.
  8. Riley, on the administrator dashboard, sees the **Items in Verification** count increment to 1 (FR-50). Riley opens the Lost report, sees every Recovery Response (Sam selected, Pat in `Submitted — Standby`), opens the verification screen (FR-45), and arranges physical verification at the desk. The acting administrator on Riley's actions is Riley (correction #36: any administrator may manage verification; each verification action records the acting administrator who performed it; there is no pre-assignment gate and no separate verifier assignment lifecycle).
  9. Maya physically inspects the candidate item and determines it is her wallet (correction #3; owner-determines). **Riley records Maya's Match Confirmed determination in FindBack** on her behalf (correction #3; admin-records) — the audit entry records `determined_by = Maya`, `recorded_by = Riley`, the determination `Match Confirmed`, and the verification timestamp (FR-48 event 18, corrected). Sam's response transitions to `Match Confirmed` (correction #16: not terminal — physical return still pending). The Lost report **remains in `Verification Pending`**. **Reporter withdrawal is now blocked** (correction #20).
  10. Physical handoff occurs. Riley confirms physical return: receiver is Maya (Lost-report owner, default per FR-45 mirrors FR-24 with Lost-report owner as default receiver; substitute rules apply if the actual receiver differs and is authorized by the owner). Riley clicks **Confirm Returned**. The Lost report transitions from `Verification Pending` to `Returned`. In the same transaction (correction #38):
     - Sam's response transitions to `Completed — Report Returned` (correction #16: the new successful terminal status reached after Match Confirmed + physical handoff + administrator Confirm Returned).
...
- **Pending User** — A registered user whose account has not yet been approved. Cannot use lost-and-found features (FR-2). Receives a confirmation page after registration; **no email is sent** at registration, on rejection, or until approval — only the account-approved email is sent on approval (FR-1, FR-3, correction #8).
...
- **Per-claim Message Thread** — The MVP-only contact channel between the claimant, the finder of the Found item the claim targets, and all administrators. See PRD-author decision D1.
- **Per-Recovery-Response Message Thread** — The MVP-only contact channel scoped to a single Recovery Response (D6, correction #10).
...
- **My Claims** — A member-facing participation view listing every Claim the authenticated member has submitted on other members' Found reports, with current Claim status, a link to the Found report, and access to the per-Claim thread (correction #16).
...
### 3.1 Lost-side Recovery Glossary (Revision 2 additions, refined in Revision 3)
...
- **Per-Recovery-Response Message Thread** — The MVP-only contact channel scoped to a **single** Recovery Response. Participants are the Lost-report owner, the responder who submitted that Recovery Response, and all administrators. Competing responders do **not** see each other's threads. See PRD-author decision D6.
...
- **No email is sent on registration, on rejection, or on any other account transition except approval** (correction #8). Email is sent only after approval via FR-27.

#### FR-2: Pending State and Restricted Access

A Pending user can log in but cannot create reports, search beyond the public summary listing, view item details, submit Claims, submit Recovery Responses, select Recovery Responses, post messages, or use any lost-and-found feature.
...
- Reject requires a short reason (free text, minimum 5 characters). The reason is recorded in the audit trail but is **not exposed** to the rejected user. **No rejection email is sent** (correction #8; the rejected user has no way to log in and therefore no way to receive in-platform messages; no out-of-platform email is sent).
- The Pending Registrations queue is the only mechanism by which administrators learn of a new registration. There is no separate in-app notification event for new registrations (correction #8).
...
- All reports, Claims, Recovery Responses, messages, and audit history owned by the deactivated user remain visible to administrators (correction #21).
...
- **Lost-report owner deactivation (correction #14):** If a Lost report's owner is deactivated, the report and history remain preserved. The deactivated owner cannot log in, post on threads, select Recovery Responses, or perform member actions.
...
**Consequences (testable):**
- Same validation rules as FR-6, except the date field uses the Found-report rules: **date found must be within the last 30 days and must not be in the future, per FR-8.** For MVP, the 30-day past bound is the authoritative requirement (FR-8 is the authoritative field definition; no further past-bound expansion or configurability is specified by the Product Brief or another authoritative requirement).
- On submit, status is **Open**, audit trail records the reporter and timestamp.
...
A member can edit their own report only while the report remains active and has not reached **Returned** or **Closed** (correction #3). Both terminal statuses block edits.
...
Hard deletion is preserved exclusively for interaction-free Open reports (correction #4). Once any Claim (Found) or Recovery Response (Lost) exists on the report, hard deletion is unavailable; the owner must Withdraw instead (FR-11).
...
- The `interaction-free Open` rule is symmetric across Found and Lost reports. There is no exception path through any other FR (this corrects a Revision 2 wording where FR-42 appeared to extend FR-10; the canonical rule lives in FR-10 and FR-42 cross-references it).

#### FR-11: Withdraw Own Report

A member may withdraw their own report. Withdrawal transitions the report to **Closed** and removes it from active listings. The withdrawal rules differ by report type (correction #5).

**Found report withdrawal (correction #5):**
...
**Lost report withdrawal (correction #5):**

- **Allowed** while the Lost report is `Open` or `Verification Pending` **before Match Confirmed has been recorded**.
- **Blocked** once Match Confirmed has been recorded (correction #20), once the report reaches `Returned`, or once the report reaches `Closed`.
...
- The owner's My Reports retrieval rule is governed by FR-49, not by FR-28 (cross-reference correction #11).
...
- **Exact location free text is a Sensitive Field.** It is visible only to the report owner, to administrators, and (where applicable per the matrix) to parties with a role on the item. **It is NOT visible to an unrelated authenticated member** (correction #18). The previous Revision 2 wording that said "visible to authenticated members per D2" was incorrect and is replaced by this FR.
- Identifying details free text on the report is also a Sensitive Field with the same restricted visibility.

#### FR-14: Search and Filter (revision 3: Verification Pending added; status filter enum explicit)
...
- **Status filter enum (correction #13, revision 3):**
...
#### FR-21: Administrator Close Action (refined against lifecycle guards, correction #22)
...
**Lost report closure (correction #22):**
...
- **`Claim Approved` → `Closed`:** Permitted only as documented exceptional administrator closure (correction #21). The Approved Claim remains `Approved` for historical accuracy; the audit trail records that physical return did not complete and why. Per-Claim threads remain read-only.
- **`Returned` → `Closed`:** **Unavailable** (Returned is terminal).
- **`Closed` → `Closed`:** **No-op**.
...
- **Audit cross-references (Found-side vs Lost-side, cleanup pass):** The audit trail records the Close action with the administrator, the documented reason, the prior report lifecycle status, and a timestamp. The audit event used depends on the report type:
...
**Returned and Closed terminal-state responsibility (correction #6):**
...
- **Closed** is reachable through:
  ...
  - **Administrator moderation or content removal** (FR-21, FR-36) — including exceptional Found-side closure after `Claim Approved` (correction #21) and exceptional Lost-side cancellation after `Match Confirmed` (correction #20).
  - **Documented exceptional administrator cancellation** (FR-21) after a Lost-side `Match Confirmed` (correction #20).
...
### 4.8 Email Notifications (revision 3 — eight total events)

**Description:** Eight MVP email events are sent total: three Found-side events (FR-27, FR-28, FR-29) plus five Lost-side Recovery Response events (FR-46). No in-app notification stream; no push notifications; no SMS. Email is the only out-of-platform notification channel. Realizes UJ-1, UJ-2, UJ-3, UJ-5.
...
- **No email is sent on registration, on rejection, or on any other account transition** (correction #8; FR-1, FR-3).
...
- **Sent to the claimant only.** No Claim-decision email is sent to the finder (correction #9; the finder's view of the Found-report status changes silently via My Reports — the finder is not the beneficiary of a Claim decision email).

#### FR-30: Unsupported Notification Channels and Events (renamed and reframed, correction #25)

The MVP supports only the eight email events enumerated in FR-27, FR-28, FR-29, and FR-46. The MVP does **not** support the following channels or events (correction #7):
...
- **No registration-rejection email** (correction #8).
- **No "lost-and-found digest" emails.**

**Consequences (testable):**
- The system does not produce any of the unsupported events above under any trigger.
- The list of supported events above is exhaustive for MVP. Any new email event introduced in a future revision must be added to FR-27, FR-28, FR-29, or FR-46 and ratified through the brief process.

**Feature-specific NFRs:** Email delivery success rate target is **95% within 5 minutes** for all eight MVP email events; failure does not block the triggering action. Validates SM-5 across all emails.
...
- The queue is scoped to Found reports only. Lost reports in `Verification Pending` belong to FR-50 (Items-in-Verification Queue) and are not mixed into this queue (correction #26).
...
**Description:** Implements PRD-author decision D2 and the unified visibility matrix (correction #18). Defines which fields are visible to which audiences, with one authoritative table per audience. Removes the contradiction in prior revisions where one FR said authenticated members see exact location while another said they do not.

**Functional Requirements:**

#### FR-37: Sensitive Fields (Authoritative Set)

The following fields are Sensitive Fields. They are hidden from viewers who are not authorized by the matrix in FR-39. Authorization is enforced at the API layer (correction #22).
...
- Claimant's reason free text on a Claim (visible only to administrators and to the claimant themselves — correction #9). The Found-report owner/finder does not see the claimant's reason.
...
**Recovery Response Sensitive Fields (revision 3):**
...
- Per-Recovery-Response thread content (correction #10; thread is not visible to anonymous users, unrelated members, or competing responders).

#### FR-38: Public Summary Fields (Authoritative — correction #7, cleanup pass)
...
- Status (lifecycle status):
  ...
  - For Lost reports (non-administrators): `Open`, `Verification Pending` only. `Returned` and `Closed` are excluded from the default view (correction #13).
...
**Note (correction #7, cleanup pass):** FR-38 is anonymous-only and covers the public summary listing set only. There is no anonymous detail page in MVP. The matrix in FR-39 governs authenticated visibility once the user has logged in. Do not describe FR-38 as covering "members who are not on the item" — that wording was incorrect and is corrected here. Do not introduce an anonymous detail page; the existing flow is `public listing → Login → authenticated detail page`.

#### FR-39: Member-Facing Visibility Matrix by Actor (Authoritative — correction #18, restructured for Found-side / Lost-side owner columns in cleanup pass)
...
**Columns (cleanup pass, owner-role split):**
...
- **Owner-role semantics (cleanup pass):** The Found-report owner / finder column and the Lost-report owner column are **explicitly split** because they have different scope semantics. A Found-report owner has ownership only on Found reports they authored; a Lost-report owner has ownership only on Lost reports they authored. For a Lost report, the Lost-report owner column governs all owner-level visibility, including normal owner fields (description, image, exact location, identifying details, reporter info, status) **and** Recovery Response-specific fields (responses, per-RR threads). For a Found report, the Found-report owner column governs all owner-level visibility, including per-Claim thread access on Claims against their Found report. There is no single "Report owner" column because no single member is simultaneously a Found-report owner and a Lost-report owner of the same report.
- **The Lost-report owner can see every Recovery Response and every per-Recovery-Response thread on their own Lost report** (correction #10). This is necessary for them to evaluate and select from the responses. The Lost-report owner is the only non-administrator party with cross-response visibility on a Lost report.
...
- Sensitive Recovery Response fields (where-found exact place, observed identifying details, candidate image) are protected against competing responders and unrelated members (corrections #33, #34).
...
- **One active response per responder per Lost report (correction #32):** A responder may have at most one **non-terminal** Recovery Response on a given Lost report. **Non-terminal Recovery Response statuses** are:
  - `Submitted`
...
#### FR-41: Recovery Response Statuses and Transitions (Authoritative — correction #16)

A Recovery Response moves through a fixed set of canonical statuses. **Standby is a derived/display condition only and is not a persisted status** (correction #2). Multiple Recovery Responses may exist on the same Lost report simultaneously; exactly one may be in `Selected for Verification` status at any time.

**Canonical Recovery Response statuses (8 statuses, correction #16):**
...
- **Withdrawn** — Terminal. Set when the responder withdraws their own Recovery Response while it is still `Submitted` and not currently `Selected for Verification` (correction #31).
...
- The only path from `Submitted` into a verified outcome is via `Selected for Verification` (correction #1).
...
Verification Pending
→ Closed                   (administrator moderation or organization removal BEFORE Match Confirmed — correction #4; reporter withdrawal BEFORE Match Confirmed permitted under FR-11; documented exceptional administrator cancellation AFTER Match Confirmed, FR-21)
```
...
- **Verification Pending → Closed (correction #4):** Two distinct closure categories are recognized:
...
- **Match Confirmed before administrator Returned (correction #20):** reporter withdrawal is no longer permitted. The flow must proceed to administrator Confirm Returned. Administrator cancellation of a Match-Confirmed Lost report is permitted only with a documented exceptional reason; the matched selected response transitions to `Resolved — Report Closed` along with all other non-terminal responses, and the audit trail records the cancellation reason and acting administrator.

#### FR-43: Owner Review, Provisional Selection, and Verification Outcomes (correction #3 owner-determines / administrator-records)
...
- **Display-only Standby (correction #2):** While one Recovery Response on the Lost report is in `Selected for Verification`, the UI may render the other `Submitted` responses as `Submitted — Standby`. The canonical persisted status remains `Submitted`.
...
- Selection is **provisional and reversible only through the Not-a-Match path**. A new selection is permitted only after the currently selected response reaches `Not a Match` and the Lost report has returned to `Open`. **Match Confirmed does NOT unlock another selection** (correction #6): once Match Confirmed has been recorded, no further response may be selected and no further new Recovery Response is accepted.
- **Verification responsibility (correction #3) — owner physically determines; administrator records:**
...
- The audit trail records every owner determination (`Match Confirmed` / `Not a Match`) with both `determined_by` (owner) and `recorded_by` (acting administrator), the selected response ID, the Lost report ID, and the timestamp. The Administrator is recorded as the system actor performing the write; the Owner is recorded as the determination-maker (FR-48 events 18 and 19, corrected).
- **Withdrawn response (correction #31):** A responder may withdraw their own Recovery Response only while it is `Submitted` and not currently `Selected for Verification`. Withdrawn is a terminal status preserved in history; the response cannot be reactivated.

#### FR-44: Recovery Response Field Set (Authoritative)

| Field | Required | Type | Constraints | Visibility (correction #33 / #34) |
|---|---|---|---|---|
...
- **Acting-administrator model (correction #36):** Any administrator may manage verification. The acting administrator on any verification action is determined by which administrator actually performs it. There is no pre-assignment gate. Another administrator may take over at any time by performing the next action; the previous acting administrator becomes the recorded actor on prior actions.
- **Concurrency (correction #37):** Simultaneous verification actions on the same Lost report are handled atomically per Lost report at the data layer (the atomic-selection rule lives at FR-41, FR-43, and the §10 concurrency NFR; it is not governed by FR-37). The first action wins; the second administrator receives a conflict message and must take no further action.
- The administrator cannot independently determine `Match Confirmed` or `Not a Match`. Those determinations are physically made by the Lost-report owner (correction #3; FR-43). The administrator's role is to manage process and record the owner's determination.
...
#### FR-46: Recovery Response Email Notifications (Lost-side; revision 3: numbered and realigned)

The Lost-report recovery flow requires the following five email events, in addition to the three Found-side events in FR-27, FR-28, FR-29. The MVP's complete email event set is **eight** events total (3 Found-side + 5 Lost-side) — do not describe the set as "five email notifications" (correction #7).
...
| 8 | Successful recovery completed (correction #17) | The responder whose selected Recovery Response transitioned to `Completed — Report Returned` | The Recovery Response transitions `Match Confirmed → Completed — Report Returned` AND the Lost report transitions `Verification Pending → Returned` (in the same transaction). The email fires only at this transition. It does NOT fire when Match Confirmed is initially recorded. |
...
**Privacy model (correction #10):** There is one message thread **per Recovery Response**, not one shared thread per Lost report. Each thread is scoped to one specific Recovery Response. Participants in that thread are:

- The Lost-report owner.
- The responder who submitted that Recovery Response.
- All administrators.

A responder can see and post on their own thread. A responder **cannot see** other responders' threads (correction #10). The Lost-report owner can see every per-Recovery-Response thread on their own Lost report. Administrators can see and post on every thread.

**Thread lifecycle (correction #11):**
...
#### FR-48: Recovery Response Audit Events (Revision 3 — events 15–30 covering full Lost-side audit trail)

The audit trail records the following Lost-side events in addition to the events in FR-31. Each event includes actor, action, target (Lost report, Recovery Response, or per-Recovery-Response thread message), timestamp, and any event-specific fields. Events 15–30 are new in Revision 3 and supersede Revision 2's 15–22 list. Acting-administrator-on-action is rolled into every action; no separate verifier assignment event exists (corrections #36, #39; cleanup pass).
...
16. **Message posted in per-Recovery-Response thread** — actor (Lost-report owner / responder / administrator), response ID, message text, timestamp. (Replaces Revision 2 event 16; scoped to the per-Recovery-Response thread.)
17. **Recovery Response selected for verification** — actor (Lost-report owner), Lost report ID, response ID, timestamp.
18. **Recovery Response determined Match Confirmed (correction #15)** — `determined_by` (Lost-report owner identity), `recorded_by` (acting administrator identity), Lost report ID, response ID, verification timestamp. (The acting administrator is recorded as the system actor performing the write; the Lost-report owner is recorded as the determination-maker.)
19. **Recovery Response determined Not a Match (correction #15)** — `determined_by` (Lost-report owner identity), `recorded_by` (acting administrator identity), Lost report ID, response ID, verification timestamp. (Same actor model as event 18.)
...
30. **Recovery Response transition to Completed — Report Returned (correction #16, individual-terminal event)** — actor (system; identifying the administrator who confirmed physical return), Lost report ID, response ID, the matched selected Recovery Response's new canonical status (`Completed — Report Returned`), timestamp. **This is the per-response terminal-event fired in the same transaction as event 26.** Event 26 covers the parent Lost report's batch child-resolution and references this event by response ID; event 30 records the individual matched selected response reaching its successful terminal status. Both events are recorded in the same transaction so the audit trail can be reconstructed either at the parent Lost-report level (event 26) or at the individual response level (event 30).

The acting administrator on every verification action is the administrator who performed that action (correction #36). There is no separate verifier assignment event.
...
### 4.13 Member Listing and Participation Views (Authoritative — correction #16, #27, #28)
...
- Reports remain retrievable in My Reports across all lifecycle statuses, including `Returned` and `Closed` (correction #28). Default status filters may hide `Returned` and `Closed`, but the owner can opt to show them.
- **No other member's reports appear here.**

**Browse (other members' reports):**

- Other members' Lost item reports (subject to FR-14 filters; includes `Verification Pending` per correction #13).
- Other members' Found item reports (subject to FR-14 filters).
- **Reports authored by the requesting member never appear in their own Browse** (correction #16).
...
- The view is independent of My Reports. **A successful Claim on another member's Found report does not transition any report authored by the member** (correction #16).
...
- The view is independent of My Reports. **A successful Recovery Response (matched and Returned) does not transition any report authored by the member** (correction #16).
...
### 4.14 Administrator Items-in-Verification Queue (Revision 3 — correction #15)
...
**Consequences (testable):**
- The queue contains only Lost reports whose status is `Verification Pending` (correction #26).
...
- **[NON-GOAL for MVP]** Responder-initiated edit of an already-submitted Recovery Response (responses can only be Withdrawn and re-submitted if eligible, not edited in place).
...
- Recovery Response email notifications — eight total events (FR-46, FR-27, FR-28, FR-29); event 8 fires only after Completed — Report Returned + Lost report Returned.
...
- **Eight email notifications**: three Found-side events plus five Lost-side Recovery Response events. There is no registration-rejection email, no standalone report-closure email, and no in-app notification stream (FR-30). Claim-rejection emails are supported under FR-29; closure emails are not.
...
- **SM-5: Email delivery (revision 3).** ≥ 95% of all triggered MVP notification emails delivered within 5 minutes. Validates FR-27, FR-28, FR-29, FR-46 (correction #23).
...
- **SM-7: Edit safety (correction #28).** 0% of edits to **Returned** or **Closed** reports succeed. Validates FR-9.
...
- **SM-10: Recovery Response turnaround (Revision 2).** Median time from Recovery Response submission to owner selection for verification ≤ 7 days. Validates FR-40, FR-43.
- **SM-11: Lost-side verification outcome recorded (Revision 2, correction #28).** 100% of Lost reports reaching **Returned** have a recorded Match Confirmed → Completed — Report Returned transition with `determined_by` (Lost-report owner) and `recorded_by` (acting administrator) audit fields, plus verification and Returned timestamps, mirroring SM-6 for the Lost side. Validates FR-43, FR-45, FR-48 events 18, 26, 30.
- **SM-12: Browse / My Reports separation (Revision 2).** 0 reports authored by the requesting member appear in that member's Browse results under any filter combination. Validates FR-49.
- **SM-13: Standby-response preservation (Revision 2).** When a Lost report reaches **Returned** or **Closed** with prior standby Recovery Responses, 100% of those non-terminal responses transition to a terminal-but-preserved status (**Resolved — Report Returned** or **Resolved — Report Closed**) in the same transaction; the matched selected response transitions to `Completed — Report Returned` on the Returned path. Validates FR-41, FR-45, FR-48.
- **SM-14: Successful-return email timing (correction #17).** 0 successful-match emails (FR-46 event 8) fire before physical handoff confirmation. The email fires only when the matched selected Recovery Response transitions to `Completed — Report Returned` and the Lost report transitions to `Returned` in the same transaction. Validates FR-46 event 8.
- **SM-15: Owner-vs-administrator audit (correction #15).** 100% of `Match Confirmed` and `Not a Match` audit events record both `determined_by = Lost-report owner` and `recorded_by = acting administrator`. Validates FR-48 events 18, 19.
...
- **OQ-3: Date lost beyond 365 days.** A member may report a wallet they lost more than a year ago. Is the 365-day bound on Lost-report `date lost` (FR-6, FR-8) and on Claim `date lost` (FR-15) a hard rule or relaxable? **Owner:** PM. **Revisit:** After first semester of operation, if data shows frequent rejections on date grounds.
...
### 8.1 Resolved / Retired Questions (correction #13)

- **OQ-4 — retired in Revision 3.** The previous open question about whether the per-Claim thread should become read-only at Returned was redundant given FR-26 already defines the per-Claim thread as read-only once the Claim is Approved or Rejected. The thread is therefore read-only before the report reaches Returned in the Approved case; the open question was already answered by an authoritative FR. Retired without being resolved at the PRD level because the answer is already in FR-26.
...
PRD-author decisions added in Revision 2, inherited from the finalized Product Brief's "deferred to PRD" lists for the Lost-side recovery flow and the listing partition:
...
- **Security.**
  ...
  - Per-Recovery-Response thread visibility is enforced at the API layer: a responder requesting another responder's thread receives HTTP 403.
...
- **Data Retention (revision 3, correction #22).**
  ...
  - **Hard deletion (revision 3):** remains limited to eligible Open reports with no relevant interaction — Found reports with no Claim, Lost reports with no Recovery Response (FR-10).
...
- **Clear (revision 3, correction #22).** Item status, Claim status, Recovery Response status, and Account status should be obvious at a glance.
...
| Three email notification events (original brief lock) | Yes (now extended — see Revision 3 rows for unified 8-event model) | FR-27, FR-28, FR-29, FR-30, FR-46, FR-48 |
| No in-app notifications or messaging (except per-claim and per-Recovery-Response threads) | Yes | §5, FR-26, FR-30, FR-47 |

### Brief Revision 2 (finalized 2026-09-15 — Recovery Response + Browse/My Reports)

| Locked brief item (Revision 2) | Present in PRD | Section / FR |
|---|---|---|
...
| Lost-side hard delete only when `Open` and has no Recovery Response | Yes (revision 3: hard delete explicitly symmetric across Found and Lost — Found requires no Claim, Lost requires no Recovery Response) | FR-10 (authoritative), FR-42 (cross-reference) |
...
### Cross-Cutting Brief Items Addressed in Revision 3 (final)
...
**Deferred decisions resolved (full set, revision 3 final):**
...
- D6 (per-Recovery-Response message thread, revision 3 final) — Thread is per Recovery Response, not per Lost report. FR-47.
...
- D8 (revision 3 final, member participation views) — `My Reports` (authored reports) / `Browse` (other members' reports) / `My Claims` (member's submitted Claims on other members' Found reports) / `My Recovery Responses` (member's submitted Recovery Responses on other members' Lost reports). FR-49.

**Out-of-scope items preserved unchanged:** See §5. Notably, automatic match suggestions, in-app notifications, real-time chat, identity verification, proof-of-ownership uploads, cross-organization sharing, and native mobile apps remain out of scope in revision 3. The revision 3 notification model explicitly enumerates the unsupported notification channels and event types in FR-30.

**Contradictions with the brief (revision 3 final, honest re-run after cleanup pass):** None detected in the final revision 3 text. The previously-identified internal contradictions — Match Confirmed being called both terminal and able-to-transition, the Lost-side owner-administrator responsibility being silent on who-records, the per-Recovery-Response vs per-Lost-Report thread framing being implied inconsistently, the withdrawal rules overlapping generically across Found and Lost, the Verification Pending closure paths being under-specified, the FR-38 vs FR-39 visibility split being ambiguous, the UJ-2 finder seeing claimant identifying details, the UJ-1 Lost-report cleanup being implicit after Found-side recovery, the stale FR-37 concurrency cross-reference for atomic Lost-side selection, the Audit-trail being miscounted as one of the four operational queues, OQ-4 being listed as open when it was actually answered by FR-26, D8 colliding with the Standby-as-derived rule, D6 listing `Match Confirmed` as a terminal Recovery Response status, the FR-49 My Recovery Responses wording listing `Match Confirmed` as terminal, the FR-39 matrix cell for "Other responders' per-Recovery-Response threads" under Lost-report owner showing N, the FR-39 matrix cell for Found-report-owner per-Claim thread access being absent while the duplicate "Per-Claim thread content on a Claim the viewer is not a party to" row appeared twice, the Verification Pending glossary describing the status as ending at Match Confirmed, the FR-48 heading and intro describing events 15-29 while event 30 was actually enumerated, the FR-48 Event 26 vs Event 30 distinction being unclear, the FR-31 event 10 Administrator Close lacking the previous lifecycle status and closure reason, the auto-rejection audit behavior collapsing three different triggers into one reason, the UJ-1 Lost-report cleanup not being an explicit final manual step, and the OQ-3 cross-references missing FR-6/FR-8 — have all been reconciled in the cleanup pass.

**Missing locked requirements (revision 3 final, honest re-run):** None detected. All original-brief items and all revision-2 brief items are resolved. The new canonical `Completed — Report Returned` status is the implementation consequence of the brief's requirement that "the administrator confirms physical Returned after the owner confirms the match" combined with the explicit requirement that the recovery process be reconstructable. The Lost-report-owner deactivation handling is the implementation consequence of the brief's `Active → Deactivated` lifecycle combined with the verification responsibility. The `determined_by`/`recorded_by` audit fields are the implementation consequence of the brief's "record who made the determination and which administrator recorded" requirement.

**Unresolved deferred decisions (revision 3 final, honest re-run):** None. All eight deferred items (D1 through D8) are resolved. OQ-4 was retired into §8.1 Resolved / Retired Questions because the answer was already provided by FR-26.

**Accidental scope expansion (revision 3 final, honest re-run):** None. Recovery Response, per-Recovery-Response messaging, the listing partition, the five Recovery Response email events, the Items-in-Verification queue, `Completed — Report Returned`, the `determined_by`/`recorded_by` audit fields, and the Lost-report-owner deactivation paths are all implementation consequences of the locked brief's verification responsibility, reporting requirements, and audit requirements, and are not scope additions.